...
package net.play5d.game.obvn.ctrler.game_ctrler {
	...
	public class GameMainLogicCtrler {
		...
		public function initlize(gameStage:GameStage, teamMap:TeamMap, map:MapMain):void {
		...
		private function renderMainLogic():void {
			...
			var displays:Vector.<IGameSprite> = _gameStage.getGameSprites();
			if (!displays) {
				return;
			}
			
			
			for each (var display:IGameSprite in displays) {
				renderGameSprite(display);
			}
		...
		private function renderGameSprite(sp:IGameSprite):void {
			...
			try {
				...
				if (sp is BaseGameSprite) {
					...
					var isInAir:Boolean = GameLogic.isInAir(bsp);
					if (isInAir) {
						bsp.applayG(GameConfig.G);
					}
					bsp.setInAir(isInAir);
				}
				...
				var errMsg:String = String(sp) + "\n";
				if (sp is FighterMain) {
					errMsg += "ID：" + (sp as FighterMain).data.id;
				}
				else if (sp is Assister) {
					errMsg += "ID：" + (sp as Assister).data.id;
				}
				else if (sp is FighterAttacker) {
					errMsg += "NAME：" + (sp as FighterAttacker).name;
				}
				else if (sp is Bullet) {
					errMsg += "ID：" + (sp as Bullet).getHitVO().id;
				}
				
				errMsg += "\n" + e.getStackTrace();
				Debugger.log("GameMainLogicCtrler.renderGameSprite :: " + errMsg);
			}
		}

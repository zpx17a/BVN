...
package net.play5d.game.obvn.fighter.ctrler {
	...
	import net.play5d.game.obvn.fighter.vo.MoveTargetParamVO;
	...
	public class FighterMcCtrler {
		...
		public var effectCtrler:FighterEffectCtrler;
		
		private var _actionCtrler:IFighterActionCtrler;
		private var _mc:FighterMC;
		private var _fighter:FighterMain;
		private var _assisterCache:Assister;
		private var _targetAssisterCache:Assister;
		...
		private var _hurtBreakFrame:uint = 0;
		...
		public function setActionCtrler(v:IFighterActionCtrler):void {
			_actionCtrler = v;
		}
		...
		public function getAttacker(name:String):FighterAttackerCtrler {
			var attacker:FighterAttacker = GameCtrler.I.getAttacker(name, _fighter.team.id);
			if (attacker) {
		...
		public function getAssister():Assister {
			return GameCtrler.I.getAssister(_fighter.team.id);
		}
		
		public function getTargetAssister():Assister {
			return GameCtrler.I.getAssister(_fighter.team.id == 1 ? 2 : 1);
		}
		...
		public function render():void {
			if (!_assisterCache) {
				_assisterCache = getAssister();
			}
			if (!_targetAssisterCache) {
				_targetAssisterCache = getTargetAssister();
			}
		...
		private function renderFloorAction():void {
			...
			// 细化投技判定
			var isSelfOnTargetLeft:Boolean = _fighter.x < target.x;
			var isTrueAction:Boolean = 
				(_actionCtrler.moveRIGHT() &&  isSelfOnTargetLeft && _fighter.direct ==  1) ||
				(_actionCtrler.moveLEFT()  && !isSelfOnTargetLeft && _fighter.direct == -1)
			;
		...
		private function checkDoWankai(action:String, attackingAct:String):Boolean {
			if (action) {
				doWanKaiAction(action);
				return true;
			}
			
			if (_mc.currentFrameName == "防御") {
				doWanKaiAction(attackingAct);
				return true;
			}
		...
		private function doWanKaiAction(action:String):void {
			if (!_mc.checkFrame(action)) {
				return;
			}
		...
		public function beHit(hitvo:HitVO, hitRect:Rectangle = null):void {
			...
			hitvo = settlementBuffBefore(hitvo);
			...
			if (_fighter.isSteelBody) {
				...
				if (_isDefense) {
					if (hitvo.isCatch() || hitvo.isReal) {
						if (hitvo.isReal) {
							trace("该伤害为真实伤害，不可防御：" + hitvo.id);
						}
...
//				}
		 ...
		 * 结算前 BUFF
		 */
		private function settlementBuffBefore(hitvo:HitVO):HitVO {
			var newHitvo:HitVO           = hitvo;
			var buffEffects:Object       = _assisterCache ? _assisterCache.buffEffects : null;
			
			if (buffEffects) {
				// 结算 属性伤害减免
				if (buffEffects.attrResistance && HitType.isAttribute(newHitvo.hitType)) {
					var attrResistance:Number = buffEffects.attrResistance;
					if (attrResistance > 1) {
						attrResistance = 1;
					}
					if (attrResistance < 0) {
					...
					var defenseHp:Number = newHitvo.power * attrResistance;
					newHitvo = newHitvo.clone();
					newHitvo.power -= defenseHp;
					
					trace("属性抗性：" + attrResistance + "，减免伤害：" + defenseHp);
				}
			}
		 ...
		 * 结算后 BUFF
		 */
		private function settlementBuffAfter(hitvo:HitVO):void {
			if (!_fighter || !_fighter.getCurrentTarget()) {
				return;
			}
			...
			var targetBuffEffects:Object = _targetAssisterCache ? _targetAssisterCache.buffEffects : null;
			if (!targetBuffEffects) {
				return;
			}
			
			// 结算 物理吸血
			if (targetBuffEffects.phyLifesteal && HitType.isPhysical(hitvo.hitType) &&
				!hitvo.isBisha() && FighterActionState.isHurting(_fighter.actionState)
			) {
				var phyLifesteal:Number = targetBuffEffects.phyLifesteal;
				if (phyLifesteal > 1) {
					phyLifesteal = 1;
				}
				if (phyLifesteal < 0) {
				...
				var addHp:Number = hitvo.getDamage() * phyLifesteal;
				target.getCtrler().addHp(addHp);
				
				trace("物理吸血：" + phyLifesteal + "，吸取血量：" + addHp);
			}
		}
		...
		private function doSteelHurt(hitvo:HitVO, hitRect:Rectangle):void {
			...
			// 钢身减伤
			_fighter.loseHp(hitvo.getDamage() * (_fighter.isSuperSteelBody ? 0.3 : 0.65));
			...
			if (hitvo && hitRect) {
				EffectCtrler.I.doSteelHitEffect(hitvo, hitRect, _fighter);
			}
		}
		...
		private function doHurt(hitvo:HitVO, hitRect:Rectangle):void {
			...
			if (hitvo && hitRect) {
				EffectCtrler.I.doHitEffect(hitvo, hitRect, _fighter);
			}
		 ...
		 * 检测能否进行 灵压爆发/替身
		 */
		public function isCanHurtBreak():Boolean {
			if (_hurtBreakFrame > 0     ||
				!_fighter.hasEnergy(50) ||
				_fighter.qi < 100)
			{
				return false;
			}
			
			var lastHitvo:HitVO   = _fighter.getLastHurtHitVO();
			var bishaHit :Boolean = lastHitvo && lastHitvo.isBisha();
			var breakHit :Boolean = _fighter.hurtBreakHit();
			var damage   :int     = _fighter.currentHurtDamage();
			
			return !bishaHit && !breakHit && !(damage > 210);
		}
		
		private function renderHurtBreak():void {
			if (!_actionCtrler.specailSkill() || !isCanHurtBreak()) {
				return;
			}
		...
		private function doDefenseHit(hitvo:HitVO, hitRect:Rectangle):void {
			...
			if (hitvo.isBreakDef) {
				// 取消固定 90 点防御消耗，改为 90%
//				defEnergy = 90;
				defEnergy = int(_fighter.energyMax * 0.9);
			}
			else {
			...
			if (hitvo && hitRect) {
				EffectCtrler.I.doDefenseEffect(hitvo, hitRect, _fighter.defenseType);
			}
		}
		...
		private function doBreakDefense(hitvo:HitVO, hitRect:Rectangle):void {
			...
			if (hitvo && hitRect) {
				...
				EffectCtrler.I.doDefenseEffect(hitvo, hitRect, _fighter.defenseType);
				EffectCtrler.I.doEffectById("break_def", effectx, effecty, _fighter.direct);
			}
		}

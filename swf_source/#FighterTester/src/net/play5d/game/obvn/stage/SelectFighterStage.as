...
package net.play5d.game.obvn.stage {
	...
	import net.play5d.game.obvn.vo.SelectCharListConfigVO;
	import net.play5d.game.obvn.vo.SelectCharListItemVO;
	import net.play5d.game.obvn.vo.SelectStageConfigVO;
	...
	public class SelectFighterStage implements IStage {
		...
		public function build():void {
			...
			SoundCtrler.I.BGM(AssetManager.I.getSound("select"));
			
			StateCtrler.I.clearTrans();
			KeyBoarder.focus();
		...
		private function addFighterItem(sv:SelectCharListItemVO):SelectFighterItem {
			...
			var isAssister:Boolean = _selectState == SELECT_STATE_ASSIST;
			...
			;
			if (!fv) {
				Debugger.log("SelectFighterStage.addFighterItem :: 没有找到角色数据：" + sv.fighterID);
				return null;
			}
			
			var si:SelectFighterItem = new SelectFighterItem(fv, sv);
			si.isAssister = isAssister;
			...
			si.addEventListener(MouseEvent.RIGHT_CLICK, selectFighterMouseHandler);
...
//		}
		...
		private function doHover(target:SelectFighterItem):void {
			if (_p1Slt && _p1Slt.enabled) {
//				if (_p1Slt.isSelected(target.fighterData.id)) {
//					return;
//				}
...
//				if (_p2Slt.isSelected(target.fighterData.id)) {
//					return;
//				}
		...
		private function doChangeMore(target:SelectFighterItem):void {
			if (_p1Slt && _p1Slt.enabled && target.hasMore) {
				_p1Slt.changeMore();
				SoundCtrler.I.sndSelect();
				return;
			}
			
			if (_p2Slt && _p2Slt.enabled && target.hasMore) {
				_p2Slt.changeMore();
				SoundCtrler.I.sndSelect();
			}
		}
		...
		private function moveSlt(slt:SelecterItemUI, x:int, y:int, fix:Boolean = true):Boolean {
			...
			if (!sf /*|| sf && slt.isSelected(sf.fighterData.id)*/) {
				if (!fix) {
					return true;
				}
				...
				if (x > slt.x) {
					...
					for (i = 0; i < _curListConfig.HCount; i++) {
						...
						if (sf /*&& !slt.isSelected(sf.fighterData.id)*/) {
							break;
						}
					}
				}
...
//			if (slt.isSelected(sf.fighterData.id)) {
//				if (up || down) {
...
//		}
		...
		private function moveToSelectFighter(slt:SelecterItemUI, sf:SelectFighterItem):void {
			...
			slt.currentSelectFighterItem = sf;
		...
		private function render():void {
			...
			if (_p1Slt && _p1Slt.enabled) {
				...
				if (GameInputer.special(type, 1)) {
					_p1Slt.changeMore(function ():void {
						SoundCtrler.I.sndSelect();
					});
				}
				if (GameInputer.select(type, 1)) {
					if (!_p1Slt.isSelected(_p1Slt.currentSelectFighterItem.fighterData.id)) {
						_p1Slt.select(playerSeltBack);
						SoundCtrler.I.sndConfrim();
					}
				}
			...
			if (_p2Slt && _p2Slt.enabled) {
				...
				if (GameInputer.special(type, 1)) {
					_p2Slt.changeMore(function ():void {
						SoundCtrler.I.sndSelect();
					});
				}
				if (GameInputer.select(type, 1)) {
					if (!_p2Slt.isSelected(_p2Slt.currentSelectFighterItem.fighterData.id)) {
						_p2Slt.select(playerSeltBack);
						SoundCtrler.I.sndConfrim();
					}
				}
...
//		}
		...
		public static function goLoadGame():void {
			...
			StateCtrler.I.transIn(MainGame.I.loadGame);
		}
